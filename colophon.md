...
My primary professional interests are in `Robotics`, `Automotive Power-train` & in the art of writing `Safety Critical Software`. I am especially excited by practical application of `Mathematics` in `Mechatronics`. As a hobby, I like to sit down with non-conventional `Mathematics` problems. In addition to one's profession, I believe every person should follow an art: I follow `Music`. 
